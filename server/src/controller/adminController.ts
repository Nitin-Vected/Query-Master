...
export const adminViewStudentListController = async (
  ...
  try {
    ...
    const totalNumberOfStudents = await userModel.countDocuments({
      role: "Student",
    });
  ...
  try {
    ...
    if (studentData) {
      ...
      response.status(StatusCodes.CREATED).json({
        ...
        message: "Student status updated successfully here is the Student List",
      });
    } else {
      return response
        .status(StatusCodes.NOT_FOUND)
...
export const adminResponseController = async (
  ...
  try {
    ...
    if (!query) {
      ...
      queryResponse.conversation = queryResponse.conversation.map(
        (conv: any) => ({
...
export const adminAuthenticationController = async (
  ...
  try {
    ...
    console.log(
      "userData : ",
      adminData,
      "  token in adminAuthenticationController : ",
      token
    );
    response.status(StatusCodes.CREATED).json({
      userData: adminData,
