...
export const userViewProfileController = async (request: any, response: express.Response) => {
    try {
        ...
        if (!email) {
            ...
            if (userData?.status) {
                response.status(StatusCodes.CREATED).json({ userData: userData, message: "This is your dersired data ..!" })
            } else {
...
export const userAddContactNumberController = async (request: any, response: express.Response) => {
    try {
        ...
        if (!userEmail) {
            ...
            if (userData?.status) {
                return response.status(StatusCodes.CREATED).json({ userData: userData, message: "Contact number updated successfully!" });
            } else {
                return response.status(StatusCodes.UNAUTHORIZED).json({ userData: null, message: "The account you are trying to access has been deactivated!" });
            }
        }
    } catch (error) {
        console.log(error);
...
export const userViewMyQueriesController = async (request: any, response: express.Response) => {
    try {
        ...
        if (myQueries) {
            return response.status(StatusCodes.CREATED).json({ myQueries: myQueries, message: "These are the recently raised queries by you ..!" });
        } else {
...
export const userAuthenticationController = async (request: any, response: express.Response) => {
    try {
        ...
        response.status(StatusCodes.CREATED).json({ userData: userData, token: token, message: "Authenntication Successfull ..!" });
    } catch (err) {
...
export const userAuthenticateJWT = async (request: any, response: express.Response, next: Function) => {
    try {
        ...
        return response.status(StatusCodes.UNAUTHORIZED).json({ message: "Invalid or expired User token" });
    }
};